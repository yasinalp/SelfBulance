...
#	$Line2D.points = $Navigation2D.get_simple_path($Agent.global_position, $Target.global_position)
	...
	$ObstacleController.global_position = $Agent.global_position
	$ObstacleController.global_rotation = $Agent.global_rotation
	...
	if control in [0,1] and not start:
		...
		for point in newpath:
			...
			if first_point:
				...
				if angle < -45:
					direction="sol"
					$ObstacleController.rotate(-PI/2)
				...
				print("obstacle rot:", $ObstacleController.rotation)
				pixel_number=prev_point.distance_to(point)
				$ObstacleController.global_position = prev_point
				var distPart1 = obstacleDetected()
				if distPart1:
					path_info.append(distPart1) # [...,dp1,"q",dp2]
					path_info.append("q") # ???
					var distPart2 = (pixel_number - distPart1 - (2 * obstacle_margin + 10))
					print("pixNum:", pixel_number, "dp1:", distPart1, "dp2:", distPart2)
					path_info.append(distPart2)
				else:
					path_info.append(pixel_number)  # [...,pn]
				prev_point=point
		to_be_sent = "["
...
func obstacleDetected():
	var obstacle = false
	var dists = [INF, INF, INF]
	var min_index
	var min_dist
	var obsDetectors = $ObstacleController.get_children()
	for obsDetector_i in obsDetectors.size():
		obsDetectors[obsDetector_i].force_raycast_update()
		if obsDetectors[obsDetector_i].is_colliding():
			obstacle = true
			dists[obsDetector_i] = (obsDetectors[obsDetector_i].get_collision_point().distance_to(obsDetectors[obsDetector_i].global_position))
	min_dist = dists.min()
	min_index = dists.find(min_dist)
	var pathdist1 = obsDetectors[min_index].get_collision_point().distance_to(obsDetectors[min_index].global_position) - obstacle_margin
	if obstacle:
		return pathdist1
	else: 
		return 0
